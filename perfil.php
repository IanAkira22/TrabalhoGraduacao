<?php
	...
	if (isset($_GET['userid']) == true){
		...
		if(DB::query('SELECT id FROM usuarios WHERE id=:userid', array(':userid'=>$loggedId)) == true){
			...
			$username = DB::query('SELECT username FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['username'];
			$email = DB::query('SELECT email FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['email'];
			$nome = DB::query('SELECT nome_completo FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['nome_completo'];
			$estado = DB::query('SELECT estado FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['estado'];
			$cidade = DB::query('SELECT cidade FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['cidade'];
			$cpf = DB::query('SELECT cpf FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['cpf'];
			$telefone = DB::query('SELECT telefone FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['telefone'];
			$ddd = DB::query('SELECT ddd FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['ddd'];
	...
	if (isset($_GET['requestid']) == true){
		
		
		if(DB::query('SELECT id FROM usuarios WHERE id=:userid', array(':userid'=>$loggedId)) == true){
			
			$profileId = DB::query('SELECT usuarios.id from usuarios, pedidos WHERE pedidos.id=:requestid AND pedidos.user_id=usuarios.id', array(':requestid'=>$_GET['requestid'])) [0]['id'];
			
			$username = DB::query('SELECT usuarios.username FROM usuarios, pedidos WHERE usuarios.id=:userid', array(':userid'=>$profileId))[0]['username'];
			$email = DB::query('SELECT email FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['email'];
			$nome = DB::query('SELECT usuarios.nome_completo FROM usuarios, pedidos WHERE usuarios.id=:userid', array(':userid'=>$profileId))[0]['nome_completo'];
			$estado = DB::query('SELECT estado FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['estado'];
			$cidade = DB::query('SELECT cidade FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['cidade'];
			$cpf = DB::query('SELECT cpf FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['cpf'];
			$telefone = DB::query('SELECT telefone FROM usuarios WHERE id=:userid', array(':userid'=>$profileId))[0]['telefone'];
				...
				<div class="row">
					...
					<div class="col-md-3">
						...
						<ul class="list-group">
							...
							<li class="list-group-item"><span><strong>Sobre mim</strong></span><br>
								<br>
								<?php
							...
							<br><li class="list-group-item"><span><strong>Meu Perfil</strong></span><br>
								<br>
								<?php
									echo("Aqui sera mostrado pedidos de procura feitos por você" . "<br>");
								?>
							</li>
									...
									echo("<ul class='list-group'>");
										...
										echo("<li class='list-group-item'>");
											...
											echo("<blockquote>");
												...
												echo("<div class='timelineposts' id=" . $r['id'] . ">");
													...
													echo('<button class="btn btn-default comment" type="button" style="box-shadow: 1px 1px 2px 0.5px grey; color:#39c514;background-image:url(&quot;none&quot;);background-color:transparent;"><span style="color:#39c514;"> Remover pedido de procura </span></button>'. '<br><br>'); 
													...
													echo("<footer>");
														...
														echo("<form action='perfil.php?requestid=" . $r['id'] . "' method='post' >
														...
														$requestId = $r['id'];
														$displaycomments = DB::query('SELECT comentarios.comentario, comentarios.posted_at, usuarios.username, usuarios.nome_completo FROM comentarios, usuarios WHERE comentarios.request_id=:requestid AND comentarios.user_id=usuarios.id ORDER BY comentarios.posted_at', array(':requestid'=>$requestId));
									...
									echo("<li class='list-group-item'>");
										...
										echo("<blockquote>");
											...
											echo("<div class='timelineposts'>");
												...
												echo("Você não fez nenhum post no momento");
